...
export const latestCompletedReview=(reviews,learnerId)=>(Array.isArray(reviews)?reviews:[]).filter(r=>r?.learnerId===learnerId&&r.status==='Completed').sort((a,b)=>String(b.reviewDate||'').localeCompare(String(a.reviewDate||''))||String(b.updatedAt||'').localeCompare(String(a.updatedAt||'')))[0]||null;
