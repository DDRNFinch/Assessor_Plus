...
export const CATCH_UP_PROGRESS=['On track','Improving','No change','Falling further behind'];
export const CATCH_UP_ACTION_STATUSES=['Completed','In progress','Not completed'];
export function halfwayDate(start,end){if(!/^\d{4}-\d{2}-\d{2}$/.test(start||'')||!/^\d{4}-\d{2}-\d{2}$/.test(end||''))return'';const a=Date.parse(`${start}T00:00:00Z`),b=Date.parse(`${end}T00:00:00Z`);if(!Number.isFinite(a)||!Number.isFinite(b)||b<a)return'';return new Date(a+Math.round((b-a)/2)).toISOString().slice(0,10)}
export const isCatchUpReview=review=>review?.reviewType==='Catch-up Review'||review?.kind==='catch-up';
export const reviewReference=reviews=>`REV${String(Math.max(0,...reviews.map(r=>Number(String(r?.id||'').replace(/\D/g,''))||0))+1).padStart(4,'0')}`;
export const reviewsForLearner=(reviews,learnerId)=>(Array.isArray(reviews)?reviews:[]).filter(r=>r?.learnerId===learnerId).sort((a,b)=>String(b?.reviewDate||'').localeCompare(String(a?.reviewDate||''))||String(b?.id||'').localeCompare(String(a?.id||'')));
export const unresolvedTargets=reviews=>(Array.isArray(reviews)?reviews:[]).flatMap(r=>(r?.targets||[]).filter(t=>t?.status!=='Completed').map(t=>({...t,sourceReviewId:r?.id||'',status:'Carried forward'})));
...
export function createCatchUpReview({reviews,learner,course,fullReview,date}){
 const outstanding=(fullReview?.targets||[]).filter(t=>t.status!=='Completed').map(t=>({id:t.id||crypto.randomUUID(),sourceReviewId:fullReview.id,action:t.action,owner:t.owner,targetDate:t.targetDate,status:'Not completed'}));
 return{id:reviewReference(reviews),learnerId:learner.id,courseId:course.course.id,pathwayId:learner.pathwayId||'',kind:'catch-up',reviewType:'Catch-up Review',reviewDate:date||fullReview?.catchUpDate||'',previousReviewDate:fullReview?.reviewDate||'',nextReviewDate:fullReview?.nextReviewDate||'',sourceFullReviewId:fullReview?.id||'',identitySnapshot:{learnerName:learner.name,qualification:course.course.title,pathway:pathwayForLearner(course,learner)?.title||''},progressCheck:'On track',previousActions:outstanding,notes:{improved:'',concern:'',beforeFullReview:''},targets:[],contactMethod:'',status:'Draft',createdAt:new Date().toISOString()}
}
export function validateCatchUpReview(review){const errors=[];if(!review.reviewDate)errors.push('Catch-up Review date is required.');if(!CATCH_UP_PROGRESS.includes(review.progressCheck))errors.push('Select a progress check.');for(const [i,t] of (review.targets||[]).entries())if(!t.action||!t.owner||!t.targetDate)errors.push(`Action ${i+1} is incomplete.`);return errors}
