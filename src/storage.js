...
/** Keep old on-device records readable without rewriting or deleting them. */
function sanitise(store,value){
...
export async function all(store){const db=await openDB();return new Promise((res,rej)=>{let r;try{r=db.transaction(store).objectStore(store).getAll()}catch(error){rej(error);return}r.onsuccess=()=>res((r.result||[]).map(value=>sanitise(store,value)));r.onerror=()=>rej(r.error)});}
export async function get(store,key){if(key===undefined||key===null||key==='')return undefined;const db=await openDB();return new Promise((res,rej)=>{let r;try{r=db.transaction(store).objectStore(store).get(key)}catch(error){rej(error);return}r.onsuccess=()=>res(sanitise(store,r.result));r.onerror=()=>rej(r.error)});}
