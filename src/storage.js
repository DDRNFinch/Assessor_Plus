const DB='assessor-plus-v01',VERSION=1;let promise;
export function openDB(){if(!promise)promise=new Promise((resolve,reject)=>{const r=indexedDB.open(DB,VERSION);r.onupgradeneeded=()=>{const d=r.result;if(!d.objectStoreNames.contains('learners'))d.createObjectStore('learners',{keyPath:'id'});if(!d.objectStoreNames.contains('assessments'))d.createObjectStore('assessments',{keyPath:'id'});if(!d.objectStoreNames.contains('media'))d.createObjectStore('media',{keyPath:'id'});if(!d.objectStoreNames.contains('settings'))d.createObjectStore('settings',{keyPath:'key'});};r.onsuccess=()=>resolve(r.result);r.onerror=()=>reject(r.error)});return promise;}
