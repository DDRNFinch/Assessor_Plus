...
export const STORES={learners:'id',assessments:'id',media:'id',settings:'key',profileAssets:'key',professionalDocuments:'id',reviews:'id',visits:'id'};
let promise;
...
export async function snapshotStores(){const entries=await Promise.all(Object.keys(STORES).map(async name=>[name,await all(name)]));return Object.fromEntries(entries)}
/** One transaction makes replacement all-or-nothing: a failed write aborts and retains current data. */
export async function replaceStores(data){const db=await openDB(),names=Object.keys(STORES);return new Promise((resolve,reject)=>{const tx=db.transaction(names,'readwrite');for(const name of names){const store=tx.objectStore(name);store['clear']();for(const value of data[name]||[])store.put(value)}tx.oncomplete=resolve;tx.onerror=()=>reject(tx.error);tx.onabort=()=>reject(tx.error||new Error('Restore transaction was aborted.'))})}
