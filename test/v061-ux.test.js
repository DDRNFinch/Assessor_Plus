...
test('V0.7.1 identifiers and unique cache agree exactly',async()=>{
  ...
  assert.equal(JSON.parse(pkg).version,'0.7.1');
  assert.equal(JSON.parse(manifest).version,'0.7.1');
  assert.match(index,/V0\.7\.1/);
  assert.match(pdf,/ASSESSOR\+ V0\.7\.1/);
  assert.match(build,/Built Assessor\+ V0\.7\.1/);
  assert.match(sw,/const CACHE='assessor-plus-v0\.7\.1'/);
