...
test('V0.8.1 identifiers and unique cache agree exactly',async()=>{
  ...
  assert.equal(JSON.parse(manifest).version,'0.8.1');
  assert.match(index,/V0\.8\.1(?:-[a-z0-9-]+)?/);
  assert.match(pdf,/ASSESSOR\+ V0\.8\.1(?:-[a-z0-9-]+)?/);
  assert.match(build,/Built Assessor\+ V0\.8\.1(?:-[a-z0-9-]+)?/);
