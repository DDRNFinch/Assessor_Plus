...
test('V0.7.7 qualification display titles are exact and optional units remain separate',async()=>{
...
test('V0.7.7 metadata, build and all offline qualifications are consistent',async()=>{
 ...
 assert.equal(JSON.parse(pkg).version,'0.7.7'); assert.equal(JSON.parse(manifest).version,'0.7.7');
 assert.match(index,/V0\.7\.7(?:-shell-20260811)?/); assert.match(build,/V0\.7\.7(?:-shell-20260811)?/);
 assert.match(sw,/const CACHE='assessor-plus-v0\.7\.7(?:-shell-20260811)?'/);
