...
test('V0.8.1 qualification display titles are exact and optional units remain separate',async()=>{
...
test('V0.8.1 metadata, build and all offline qualifications are consistent',async()=>{
 ...
 assert.equal(JSON.parse(pkg).version,'0.8.1'); assert.equal(JSON.parse(manifest).version,'0.8.1');
 assert.match(index,/V0\.8\.1(?:-[a-z0-9-]+)?/); assert.match(build,/V0\.8\.1(?:-[a-z0-9-]+)?/);
 assert.match(sw,/const CACHE='assessor-plus-v0\.8\.1(?:-[a-z0-9-]+)?'/);
