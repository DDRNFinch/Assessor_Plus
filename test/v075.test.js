...
test('V0.7.6 metadata is consistent and protected course data and mapping inventories are unchanged',async()=>{
 assert.equal(JSON.parse(await read('package.json')).version,'0.7.6');assert.equal(JSON.parse(await read('manifest.webmanifest')).version,'0.7.6');assert.match(await read('index.html'),/V0\.7\.6(?:-shell-20260811)?/);assert.match(await read('src/app.js'),/Assessor\+ V0\.7\.6(?:-shell-20260811)?/);assert.match(await read('src/pdf.js'),/ASSESSOR\+ V0\.7\.6(?:-shell-20260811)?/);assert.match(await read('sw.js'),/const CACHE='assessor-plus-v0\.7\.6(?:-shell-20260811)?'/);assert.match(await read('scripts/build.js'),/V0\.7\.6(?:-shell-20260811)?/);
